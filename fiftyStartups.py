...
def normalize_feature(featureLabelIn):
    ...
    print(f"{featureLabelIn} Max Value is", maxVal)
...
def plot_features(FeatureIdxIn):
    ...
    # Plot the linear fit
    plt.plot(
        x_train[selectedFeature],
        (optimalW[0][FeatureIdxIn] * x_train[selectedFeature] + optimalB),
        c="g",
        label=f"Fitted line for {selectedFeature}",
    )
    plt.scatter(x_test[selectedFeature], yPred, c="b", label="Model Predictions")
    # Create a scatter plot of the data.
    plt.scatter(
        x_test[selectedFeature], y_test, marker="x", c="r", label="Target Labels"
    )
    # Set the title
...
#%% serialize the model
pickle_output = open("startup_50.pkl", "wb")
